            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" x-model="form.playbook" @change="loadContent">
                        ...
                        <option value="{{ $pb }}">{{ basename($pb) }} <span style="color:var(--text-muted)">{{ dirname($pb) }}</span></option>
                        @endforeach
                    </select>
                    <div class="form-hint">From {{ config('ansible.playbooks_dir') }}</div>
                </div>

                <div class="form-group">
                    <label class="form-label">Inventory</label>
                    <input type="text" class="form-input" x-model="form.inventory"
                        placeholder="{{ config('ansible.inventory_default') }}">
    ...
    <div>
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('logs.jobs') }}" class="btn btn-sm btn-secondary ml-auto">All Jobs</a>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-mono text-sm" style="color:var(--text-primary)">{{ basename($job->playbook) }}</td>
                            <td><span class="badge badge-{{ $job->status }}">{{ $job->status }}</span></td>
                            <td class="text-mono text-xs">
                                @if($job->hosts_ok > 0)
                                    <span class="text-green">ok={{ $job->hosts_ok }}</span>
                                    <span class="text-yellow"> chg={{ $job->hosts_changed }}</span>
                                @else —
                                @endif
                            </td>
...
function playbookRunner() {
    return {
        ...
        commandPreview() {
            ...
            const inv = this.form.inventory || '{{ config('ansible.inventory_default') }}';
            parts.push('-i', inv);
